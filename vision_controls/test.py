...
GRIPPER_LENGTH = 0.02   # flange-to-tip, measured

def cam_to_base(point_cam):
    """RealSense camera frame (metres) -> robot base frame (metres)."""
    p = np.array([point_cam[0], point_cam[1], point_cam[2], 1.0])
    return (T_CAM2BASE @ p)[:3]
...
try:
    while True:
        ...
        for (label, conf, x1, y1, x2, y2) in detections:
            ...
            x_3d, y_3d, z_3d = cam_to_base(point_3d)
            z_3d += GRIPPER_LENGTH     # aim the flange above the fruit, tip lands on it
            ### -----------------------------------
